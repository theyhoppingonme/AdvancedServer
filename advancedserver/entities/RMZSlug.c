...
bool slug_init(Server* server, Entity* entity)
{
	...
	int num = rand() % 100;
	if (num < g_config.states.gameplay.entities_misc.map_specific.ravine_mist.slugs.red_ring_chance)
        slug->ring = SLUG_REDRING;
	else if (num >= g_config.states.gameplay.entities_misc.map_specific.ravine_mist.slugs.red_ring_chance
		&& num < g_config.states.gameplay.entities_misc.map_specific.ravine_mist.slugs.red_ring_chance + g_config.states.gameplay.entities_misc.map_specific.ravine_mist.slugs.ring_chance)
		slug->ring = SLUG_RING;
	else
