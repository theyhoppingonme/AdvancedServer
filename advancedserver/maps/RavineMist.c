...
bool rmz_checkstate(Server* server)
{
	...
	if (server->game.time_sec <= TICKSPERSEC - 10 && !g_config.states.gameplay.banana.disable_timer)
	{
		uint8_t total = 7 - game_find(server, NULL, "shard", 7);
...
bool rmz_init(Server* server)
{
	...
	if (g_config.states.gameplay.entities_misc.map_specific.ravine_mist.slugs.enabled) {
    ...
    for (int i = 0; i < g_config.states.gameplay.entities_misc.map_specific.ravine_mist.shards.amount; i++)
...
bool rmz_tick(Server* server)
{
	...
	{
        if (server->game.time_sec <= g_config.states.gameplay.ring_appearance_timer && server->game.bring_state < BS_DEACTIVATED && !g_config.states.gameplay.banana.disable_timer)
			game_bigring(server, BS_DEACTIVATED);

        if (server->game.time_sec <= g_config.states.gameplay.escape_time && server->game.bring_state < BS_ACTIVATED)
		{
            if(
