		...
		case CLIENT_CHAT_MESSAGE:
		{
			...
			switch (hash)
			{
                ...
                case CMD_ENDVOTE:
                {
                    ...
                    if (v->server->lobby.vote.countdown <= 0)
                    {
                        RAssert(server_send_msg(v->server, v->peer, CLRCODE_RED "there's no vote ongoing"));
                        break;
                    }

                    v->server->lobby.vote.ongoing = false;
                    v->server->lobby.vote.votecnt = 0;
                    v->server->lobby.vote.countdown = 0;
                    server_broadcast_msg(v->server, 0, CLRCODE_RED "vote dismissed by a moderator");
                    break;
                }
			}
