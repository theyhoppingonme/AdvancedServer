...
{
    if (!g_config.states.gameplay.banana.disable_timer && !g_config.states.gameplay.gmcycle.overhell) {
        if (server->game.time_sec <= g_config.states.gameplay.ring_appearance_timer && server->game.bring_state < BS_DEACTIVATED)
...
{
    server->game.time_sec = g_config.states.gameplay.banana.disable_timer ? 0 : (uint16_t)(seconds + (((server_ingame(server) - 1) * mul)));
	return true;
}
